...
{ //sub_path does not own the string
  const char *begin;
  const char *end;
} sub_path;
...
static expression compile_expression(const char *begin, const char *end)
{
  ...
  if (logic_first[0] != logic_first[1])
    fail(); //&& or ||
  result.tag = exp_logic;
  result.logic = NEW(logic_exp);
...
static void logic_exp_free(logic_exp *exp)
{
  ...
  if (exp->operator> logic_negtive)
    expression_free(exp->oprand[1]);
  DELETE(exp);
}
...
static path_result json_path_get_property(json_t *curr, const char *begin, const char *end)
{
  ...
  if (segn == 1 && !seg_filled[0])
  { // select named properties
    result.is_collection = FALSE;
  ...
  { //[i:j]([:j] [i:]) or [i:j:k] or [-i:]
    ...
    if (!rev)
    {
      if (seg_filled[0] && seg_int[0] >= 0)
        index_begin = seg_int[0];
      if (segn == 2 && seg_filled[1] && ((size_t)seg_int[1]) < index_end)
        index_end = seg_int[1];
      else if (segn == 3 && seg_filled[2] && ((size_t)seg_int[2]) < index_end)
        index_end = seg_int[2];
    ...
    {
      long index_begin_l = index_end + seg_int[0]; //index_begin=index_end-i
      if (index_begin_l >= 0)
        index_begin = index_begin_l;
    }
    size_t i;
...
static json_t *json_path_eval(json_t *root, json_t *curr, const char *begin, const char *end)
{
  ...
  if (begin[0] == '@')
    ...
    return json_path_get_impl(root, curr_p, begin, end).result; //default to current node
}
...
{
  if (op == comparison_nocmp || op == comparison_error || op == comparison_reg)
    return NULL; //should not happen
  if (!lhs || !rhs)
    return json_false();
  if (json_is_array(lhs) || json_is_array(rhs) || json_is_object(lhs) || json_is_object(rhs))
    return json_false();

  if (json_is_null(lhs) || json_is_null(rhs))
  { //null should not be equal to anything other than null
    if (op != comparison_eq && op != comparison_ne)
      return json_false();
  ...
  if (json_is_number(lhs) || json_is_number(rhs) || (op != comparison_eq && op != comparison_ne))
  { //numeric compare
    if (json_is_real(lhs) || json_is_real(rhs))
    {
      double lhs_d = json_to_double(lhs);
      double rhs_d = json_to_double(rhs);
      if (isnan(lhs_d) || isnan(rhs_d))
        return json_false();
    ...
    else
    {
      ...
      if (lhs_d > INT_MAX || rhs_d > INT_MAX)
        return json_false();
      return json_boolean(comp_diff(lhs_d - rhs_d, op));
    }
  }
  ...
  else if (json_is_string(lhs) && json_is_string(rhs))
  { // string compare
...
static json_t *execute_exp(expression exp, json_t *root, json_t *curr)
{
  if (exp.tag == exp_logic)
  {
    ...
    if (logic->operator== logic_positive || logic->operator== logic_negtive)
    {
      ...
      json_decref(ret);
      if (logic->operator== logic_negtive)
        b = !b;
                                      ...
                                      const char *end)
{
  if (!curr.result || path == end ||
      *path == '\0')
  {                           // no selection to do. return original collection
